...
export default function StudentSupportDonationPage() {
  ...
  const allocations = [
    { icon: 'menu_book',         title: t('poorEducational'), desc: t('poorEducationalDesc'), pctVal: ALLOC_PCTS[0] },
    { icon: 'home',              title: t('poorAccommodation'), desc: t('poorAccommodationDesc'), pctVal: ALLOC_PCTS[1] },
    { icon: 'health_and_safety', title: t('poorHealthcare'), desc: t('poorHealthcareDesc'), pctVal: ALLOC_PCTS[2] },
  ];
  ...
  useEffect(() => {
    donationService.getProjects()
      .then(({ data }) => {
        const p = data.find((p) => p.projectType === 'Poor Student Support');
    ...
    <main className="w-full pt-8 pb-20">
      <div className="max-w-container-max mx-auto px-4 md:px-6">
        ...
        <div className="flex items-center gap-2 text-sm text-text-muted font-inter mb-8">
          ...
          <span className="text-primary-container font-semibold">{t('campaignPoorStudentTitle')}</span>
        </div>
