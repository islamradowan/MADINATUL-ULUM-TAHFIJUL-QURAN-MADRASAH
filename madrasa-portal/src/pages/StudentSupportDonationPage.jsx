...
export default function StudentSupportDonationPage() {
  ...
  const ALLOC_PCTS = [0.55, 0.30, 0.15];
  const allocations = [
    { icon: 'menu_book',         title: t('studentAlloc1'), desc: t('studentAlloc1Desc'), pctVal: ALLOC_PCTS[0] },
    { icon: 'home',              title: t('studentAlloc2'), desc: t('studentAlloc2Desc'), pctVal: ALLOC_PCTS[1] },
    { icon: 'health_and_safety', title: t('studentAlloc3'), desc: t('studentAlloc3Desc'), pctVal: ALLOC_PCTS[2] },
  ];
  ...
  useEffect(() => {
    donationService.getProjects()
      .then(({ data }) => {
        const p = data.find((p) => p.projectType === 'Student Support');
  ...
  async function handleDonate(e) {
    e.preventDefault();
    if (!amount || Number(amount) <= 0) { setError(t('studentErrAmount')); return; }
  ...
  const stats = [
    { label: t('studentTotalDonors'), value: donors.toLocaleString() },
    { label: t('studentAvgGift'),     value: donors > 0 ? `৳${Math.round(raised / donors).toLocaleString()}` : '—' },
    { label: t('studentRemaining'),   value: `৳${(GOAL - raised).toLocaleString()}` },
    { label: t('studentOnTrack'),     value: pct >= 100 ? t('studentFunded') : t('studentOnTrack'), green: true },
  ];
    ...
    <main className="w-full pt-8 pb-20">
      <div className="max-w-container-max mx-auto px-4 md:px-6">
        ...
        <div className="flex items-center gap-2 text-sm text-text-muted font-inter mb-8">
          <Link to={PATHS.DONATE} className="hover:text-primary-container transition-colors">{t('studentBreadcrumbParent')}</Link>
          <span className="material-symbols-outlined text-sm">chevron_right</span>
          <span className="text-primary-container font-semibold">{t('studentBreadcrumbCurrent')}</span>
        </div>
          ...
          <div className="h-64 bg-primary-container relative overflow-hidden">
            ...
            <div className="absolute bottom-8 left-8 right-8">
              <div className="flex items-center gap-3 mb-2">
                <span className="material-symbols-outlined text-secondary-fixed text-sm">volunteer_activism</span>
                <span className="text-xs font-semibold text-secondary-fixed tracking-widest uppercase font-inter">{t('studentGoalTitle')}</span>
              </div>
              <h1 className="text-3xl md:text-5xl font-bold font-manrope text-white mb-2">{t('studentHeroTitle')}</h1>
              <p className="text-base text-surface-container-high max-w-2xl font-inter">{t('studentHeroDesc')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          <div className="lg:col-span-2 bg-surface-base rounded-xl p-8 shadow-ambient border border-border-subtle relative overflow-hidden">
            ...
            <div className="flex justify-between text-xs font-semibold text-text-muted mb-8 relative z-10 font-inter">
              <span>{pct}% {t('studentFunded')}</span>
              <span>৳{(GOAL - raised).toLocaleString()} {t('studentRemaining')}</span>
            </div>
          ...
          <div className="bg-primary-container rounded-xl p-8 shadow-ambient-lg flex flex-col justify-center text-center">
            <span className="material-symbols-outlined text-secondary-fixed text-5xl mb-4 mx-auto icon-fill">school</span>
            <h3 className="text-xl font-semibold font-manrope text-white mb-4">{t('studentFormTitle')}</h3>
            <p className="text-sm text-primary-fixed-dim mb-8 font-inter italic">
              {t('studentHeroDesc')}
            </p>
            ...
            <form onSubmit={handleDonate} className="space-y-3">
              <input
                type="text"
                placeholder={t('studentLabelName')}
                value={name}
              ...
              >
                {loading ? t('studentProcessing') : t('studentFormTitle')}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {allocations.map(({ icon, title, desc, pctVal }) => (
