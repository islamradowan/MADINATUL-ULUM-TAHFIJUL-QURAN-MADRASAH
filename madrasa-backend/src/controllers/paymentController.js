...
// Helper: create Zakat record from a transaction
async function createZakatFromTransaction(transaction) {
...
// Helper: create Donation record from a transaction
async function createDonationFromTransaction(transaction) {
...
const initPayment = async (req, res, next) => {
  try {
    const { type, amount, donorName, donorEmail, donorPhone, projectType, allocationType, paymentMethod, donorId } = req.body;
    ...
    if (apiResponse?.GatewayPageURL) {
      ...
      res.json({ success: true, gatewayUrl: apiResponse.GatewayPageURL, transactionId });
    } else {
  ...
  try {
    ...
    if (validation.status === 'VALID' || validation.status === 'VALIDATED') {
      ...
      if (transaction.type === 'donation' && !transaction.donationId)
        await createDonationFromTransaction(transaction);
      else if (transaction.type === 'zakat' && !transaction.zakatId)
        await createZakatFromTransaction(transaction);
...
const handleFail = async (req, res) => {
  try {
    ...
    const transaction = await Transaction.findOne({ transactionId: tran_id });
    if (transaction) { transaction.status = 'Failed'; await transaction.save(); }
    res.redirect(`${FRONTEND_URL}/payment/fail?tran_id=${tran_id}`);
  } catch (err) {
...
const handleCancel = async (req, res) => {
  try {
    const { tran_id } = req.method === 'POST' ? req.body : req.query;
    const transaction = await Transaction.findOne({ transactionId: tran_id });
    if (transaction) { transaction.status = 'Cancelled'; await transaction.save(); }
